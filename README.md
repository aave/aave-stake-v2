...
- Deploy the aaveuistaking helper `HARDHAT_NETWORK=goerli npx hardhat deploy-AaveStakingHelper` with the proxy of the staking contract
- Deploy `HARDHAT_NETWORK=goerli npx hardhat deploy-StakeUIHelper ` with the aave oracle and proxy contract set
- Initialize the stakedAave contract with the admin address and correct stakedAaveImpl and stakedAaveProxy addresses `HARDHAT_NETWORK=goerli npx hardhat initialize-StakedAave --admin ADMIN_ADDRESS`
