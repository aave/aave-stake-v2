...
makeSuite('StakedAave V2. Redeem', (testEnv: TestEnv) => {
  it('Reverts trying to redeem 0 amount', async () => {
    ...
    await expect(
      stakedAaveV2.connect(staker.signer).redeem(staker.address, amount)
  ...
  it('User 1 stakes 50 AAVE', async () => {
    ...
    await waitForTx(await aaveToken.connect(staker.signer).approve(stakedAaveV2.address, amount));
    await waitForTx(await stakedAaveV2.connect(staker.signer).stake(staker.address, amount));
  });
  ...
  it('User 1 activates the cooldown, but is not able to redeem before the COOLDOWN_SECONDS passed', async () => {
    ...
    const startedCooldownAt = new BigNumber(
      await (await stakedAaveV2.stakersCooldowns(staker.address)).toString()
    );
  ...
  it('User 1 activates the cooldown again, and tries to redeem a bigger amount that he has staked, receiving the balance', async () => {
    ...
    const stakedAaveBalanceBefore = (await stakedAaveV2.balanceOf(staker.address)).toString();
    await stakedAaveV2.connect(staker.signer).redeem(staker.address, amount);
    const aaveBalanceAfter = new BigNumber((await aaveToken.balanceOf(staker.address)).toString());
    const stakedAaveBalanceAfter = (await stakedAaveV2.balanceOf(staker.address)).toString();
